...
## Токены и прототип токена
Источник: `module/system.js`

- При создании актёра/НПС выставляются дефолты прототипа токена:
  - `actorLink = true`
  - `displayBars = OWNER` (шкала здоровья видна владельцу)
  - `bar1.attribute = attributes.hp`
- При изменении имени актёра обновляется `prototypeToken.name`.
- При переименовании актёра синхронизируются имена всех **связанных** токенов на сценах.
- При создании токена автоматически подтягиваются имя и портрет актёра.

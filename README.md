...
- **Доп. кубы** (extraDice): ручной модификатор пула в диалогах проверки/атаки/защиты.
  - положительное значение увеличивает пул
  - отрицательное значение уменьшает пул
  - итоговый пул всегда ограничен диапазоном `1..20`
...
- `conditionRollLuck` / `conditionRollDice` — броски Самочувствия
- `attentionRollLuck` / `attentionRollDice` — броски Внимания
- `movementRollLuck` / `movementRollDice` — броски Движения
- `combatRollLuck` / `combatRollDice` — броски Сражения
- `thinkingRollLuck` / `thinkingRollDice` — броски Мышления
- `communicationRollLuck` / `communicationRollDice` — броски Общения
- `attackRollLuck` / `attackRollDice` — броски атак (оружие/способности)
- `rollLuck` — все броски: преимущество/помеха
...
- `speed` — скорость
- `weaponLuck` — атака оружием: преимущество/помеха
- `dodgeLuck` / `dodgeDice` — уклонение
- `blockLuck` / `blockDice` — блок

Для ключей `*Luck`:
- положительное значение = преимущество
- отрицательное значение = помеха
...
2) В том же диалоге можно указать **Доп. кубы** (включая отрицательные).
3) Бросок атаки = пул атрибута с учётом эффектов и `Доп. кубов`.
4) Урон: базовый урон оружия + успехи атаки.
5) В чат приходит карточка атаки с кнопкой **Защита**.
...
- В диалоге защиты доступно поле **Доп. кубы** (может быть отрицательным), оно применяется к пулу защиты.
...
- Кнопка **Применить урон** (только ГМ) динамически внедряется в карточку атаки после выбора защиты для каждой цели.
- ГМ видит итоговый урон и может применить его одним нажатием прямо из основной карточки атаки.
